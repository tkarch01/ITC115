...
/* MovieDB simulates access to database of movies by title and category 
 * Contains two methods that put data into array lists:
 * getAllMovies loads all movies into arraylist allMovies
 * getMoviesOfCategory(arg) loads movies from allMovies with category
 * of arg
 */
public class MovieDB {
	
	/* provide an array list of all movies */
	public static ArrayList<Movie> getAllMovies() {
		ArrayList<Movie> allMovies = new ArrayList<>(100);
	...
	/* provide an array list of movies selected by category */
	public static ArrayList<Movie> getMoviesOfCategory(String movieCategory) {
		ArrayList<Movie> movies = new ArrayList<>(); 

		for (Movie movie : getAllMovies()){
			if (movie.getcategory().equalsIgnoreCase(movieCategory)){ 
			movies.add(movie);
			}
		}
		return movies;	
